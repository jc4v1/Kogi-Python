...
def read_event_mapping_csv(file_path: str) -> Dict[str, Set[str]]:
    """
    Read CSV with columns: Event, Intentional Element
    Returns mapping: { event: {intentional_elem, ...} }
    If a row has an event but no intentional element the value will be an empty set.
    """
    ...
    with open(file_path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if event not in mapping:
                mapping[event] = set()
            if element:
                mapping[event].add(element)
    return mapping


def event_mapping_for_petri_net(event_map: Dict[str, Set[str]], petri_net) -> Dict[str, Set[str]]:
    """
    ...
    `event_map`, an empty set is used.
    ...
    Example:
      ...
      resulting mapping for a transition with name 't2' and label 'e1' will be
            {'t2': {'T1'}}
    """
    mapping_by_transition: Dict[str, Set[str]] = {}
    ...
    for t in getattr(petri_net, 'net').transitions:
        ...
        if label and label in event_map:
            mapping_by_transition[t.name] = set(event_map[label])
        elif t.name in event_map:
            mapping_by_transition[t.name] = set(event_map[t.name])
        else:
            mapping_by_transition[t.name] = set()
